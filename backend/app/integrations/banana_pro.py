...
class BananaProClient(AIImageClientBase):
    """Banana Pro API 客户端"""
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        # Use config values or fall back to settings (note: currently hardcoded in original)
        self.api_key = self.get_config_value("api_key") or settings.BANANA_PRO_API_KEY
        self.base_url = self.get_config_value("base_url") or settings.BANANA_PRO_BASE_URL
        self.model = self.get_config_value("model") or settings.BANANA_PRO_MODEL
    ...
    async def generate_image_with_metadata(
        ...
        async with httpx.AsyncClient(timeout=self.timeout) as client:
            ...
            if response.status_code != 200:
                ...
                raise Exception(f"Banana Pro API error: {response.status_code} - {error_text}")
